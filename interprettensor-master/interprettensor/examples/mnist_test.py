...
flags.DEFINE_integer("test_every", 60,'Number of steps to run trainer.')
flags.DEFINE_float("learning_rate", 0.001,'Initial learning rate')
flags.DEFINE_float("dropout", 0.9, 'Keep probability for training dropout.')
flags.DEFINE_string("data_dir", 'data','Directory for storing data')
flags.DEFINE_string("summaries_dir", 'mnist_test_logs','Summaries directory')
...
#flags.DEFINE_string("checkpoint_dir", 'mnist_test_model','Checkpoint dir')
flags.DEFINE_string("checkpoint_dir", 'mnist_test_model','Checkpoint dir')
flags.DEFINE_string("checkpoint_reload_dir", 'mnist_test_model','Checkpoint dir')
...
def train():
  ...
  with tf.Session(config=config) as sess:
    ...
    if FLAGS.reload_model:
        ...
        #npy_files = np.load('mnist_test_model/model.npy', encoding='bytes')
    ...
    if FLAGS.relevance:
        ...
        images = standard_d[0].reshape([FLAGS.batch_size,28,28,1])
