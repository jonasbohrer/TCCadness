...
class DEvol:
    ...
    def __init__(self, genome_handler, data_path=""):
        ...
        self.datafile = data_path or (datetime.now().ctime().replace(" ", "").replace(":", "") + '.csv')
        self._bssf = -1
        self.counter = 0
    ...
    def run(self, dataset, num_generations, pop_size, epochs, train_size=None, fitness=None,
            metric='accuracy', replace=0.9):
        """
        ...
        (self.x_train_orig, self.y_train_orig), (self.x_test, self.y_test) = dataset

        # generate and evaluate initial population
        self.x_train = self.x_train_orig[0:train_size]
        self.y_train = self.y_train_orig[0:train_size]
        ...
        for gen in range(1, num_generations):
            self._bssf = -1
            self.x_train = self.x_train_orig[(gen)*train_size:(gen+1)*train_size]
            self.y_train = self.y_train_orig[(gen)*train_size:(gen+1)*train_size]

            members = self._reproduce(pop, gen, replace)
            pop = self._evaluate_population(members,
                                            epochs,
                                            fitness,
                                            gen,
    ...
    def _reproduce(self, pop, gen, replace):
        members = []

        # 95% of population from crossover
        for _ in range(int(len(pop) * replace)):
        ...
        members += pop.get_best(len(pop) - int(len(pop) * replace))
        return members

    def _evaluate(self, genome, epochs, igen, imem):
        ...
        self._record_stats(model, genome, loss, accuracy, igen, imem)

        return model, loss, accuracy

    def _record_stats(self, model, genome, loss, accuracy, igen, imem):
        with open(self.datafile, 'a') as csvfile:
            writer = csv.writer(csvfile, delimiter=',',
                                quotechar='"', quoting=csv.QUOTE_MINIMAL)
            row = [igen, imem] + list(genome) + [loss, accuracy]
            writer.writerow(row)

        met = loss if self._metric == 'loss' else accuracy
        model.save('models/{}_{}.h5'.format(igen, imem))
        if (self._bssf is -1 or
                self._metric_op(met, self._bssf) and
                accuracy is not 0):
            try:
                os.remove('models/best-model.h5')
            except OSError:
                pass
            self._bssf = met
            model.save('models/gen4/best-model-{}.h5'.format(igen))
    ...
    def _evaluate_population(self, members, epochs, fitness, igen, ngen):
        ...
        for imem, mem in enumerate(members):
            ...
            res = self._evaluate(mem, epochs, igen, imem)
